...
export class RepoScanner {
  ...
  private allowedExtensions: string[] = []; // <-- NUEVO: Almacena las extensiones permitidas
  ...
  /**
   * NUEVO: Configura las extensiones de la Lista Blanca.
   * Transforma entradas como "ts, html, css" en un array limpio: [".ts", ".html", ".css"]
   */
  public setExtensionFilter(extensions: string): void {
    if (!extensions || extensions.trim() === '') return;
  ...
  private async *traverse(currentDir: string, token: CancellationToken): AsyncGenerator<string> {
    ...
    for await (const entry of dir) {
      ...
      if (entry.isDirectory()) {
        ...
        // NUEVO: Filtro Whitelist por extensiones (solo afecta a los archivos)
        if (this.allowedExtensions.length > 0) {
          const hasValidExtension = this.allowedExtensions.some(ext => entry.name.endsWith(ext));
          if (!hasValidExtension) {
            continue; // Si no coincide la extensión, lo descartamos al instante sin gastar RAM
          }
        }
